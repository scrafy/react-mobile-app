...
const categoriesByCenter = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            try {
                ...
                if (search.centerId === undefined) {
                    res.statusCode = 400;
                    res.json({ error: 'Bad Request' });
                } else {
                    resp = await useCase.getCategoriesByCentreUseCase().getCategoriesByCentre(search.centerId, req.cookies["session"]);
...
export default categoriesByCenter;